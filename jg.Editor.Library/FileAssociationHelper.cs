...
namespace jg.Editor.Library
{
    public class FileAssociationHelper
    {
        ...
        public static void Unregister(string progID, params string[] fileExtensions)
        {
            Array.ForEach(fileExtensions,
                assoc => UnregisterFileAssociation(progID, assoc));
            UnregisterProgId(progID);
        }

        public static void Register(string progID, string appID, string openWith, string editWith, params string[] fileExtensions)
        {
            //First of all, unregister:
            Unregister(progID, fileExtensions);

            RegisterProgId(progID, appID, openWith, editWith);
            Array.ForEach(fileExtensions,
        ...
        private readonly static RegistryKey classesRoot = Registry.CurrentUser.OpenSubKey(@"Software\Classes", true);

        private static void RegisterProgId(string progId, string appId, string openWith,string editWith)
        {
            RegistryKey progIdKey = classesRoot.CreateSubKey(progId);
            progIdKey.SetValue(string.Empty, string.Empty);
            RegistryKey shell;
            RegistryKey iconKey = progIdKey.CreateSubKey("DefaultIcon");
            iconKey.SetValue("", openWith.Substring(0, editWith.LastIndexOf("\\")) + "\\Image\\Icon.ico\"");
            iconKey.Close();

            shell = progIdKey.CreateSubKey("shell");
            shell = shell.CreateSubKey("Open");
            shell = shell.CreateSubKey("Command");
            shell.SetValue(String.Empty, openWith);
            shell.Close();
            shell = progIdKey.CreateSubKey("shell");
            shell = shell.CreateSubKey("Edit");
            shell = shell.CreateSubKey("Command");
            shell.SetValue(String.Empty, editWith);
        ...
        {
            RegistryKey openWithKey = classesRoot.CreateSubKey(extension);
            openWithKey.SetValue(string.Empty, progId);
            openWithKey.Close();
        }
