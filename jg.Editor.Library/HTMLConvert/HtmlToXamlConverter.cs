...
namespace jg.HTMLConverter
{
    ...
    public static class HtmlToXamlConverter
    {
        ...
        {
            const string flag1 = "<section";
            const string flag2 = ">";

            const string flag3 = "#";
            const string flag4 = "\"";
            const string flag5 = "</Section>";
            const string flag6 = " Background=\"{0}\"";
            string background = "";
            string result = "";

            int start = content.ToLower().IndexOf(flag1);
            int end = content.Substring(start).ToLower().IndexOf(">") + 1;
            string a = content.Substring(start, end);

            result = content.Substring(0, start) + content.Substring(start + end);
            result = result.Replace(flag5, "");
            start = a.IndexOf(flag3);
            end = a.Substring(start).IndexOf(flag4);


            background = a.Substring(start, end);

            start = result.IndexOf(flag2);
            result = result.Substring(0, start) + string.Format(flag6, background) + result.Substring(start);
